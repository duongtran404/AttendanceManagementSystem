...
@section('content')
    <h1 class="text-center text-dark">Edit Class</h1>
    <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
        {{ Breadcrumbs::render('edit class', $class->id) }}
    </nav>
    <form class="form mt-5" action="{{ route('editClass', [$class->id]) }}" method="post">
        ...
        <div class="form-group mt-3">
            ...
            <input type="date" name="begin_date" id="begin_date" class="form-control"
                value="{{ date('Y-m-d', strtotime($class->begin_date)) }}">
            @error('begin_date')
                <span class="d-block fs-6 text-danger mt-2"> {{ $message }} </span>
            @enderror
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <input type="date" name="end_date" id="end_date" class="form-control"
                value="{{ date('Y-m-d', strtotime($class->end_date)) }}">
            @error('end_date')
                <span class="d-block fs-6 text-danger mt-2"> {{ $message }} </span>
            @enderror
        </div>
