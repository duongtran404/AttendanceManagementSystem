...
<div>
    <div>
        <a class="btn btn-primary mt-3" href="{{ route('createTeacher') }}">New Teacher</a>
    ...
    <table class="table">
        ...
        @foreach ($teachers as $item)
            <tr class="text-center">
                ...
                <td>{{ $item->notes }}</td>
                <td>
                    <form action="{{ route('deleteTeacher', [$item->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <a class="btn btn-primary" href="{{ route('editTeacher', $item->id) }}">Edit</a>
